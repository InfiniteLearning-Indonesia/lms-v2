...
export default async function PeoplePage({ params }: { params: Promise<{ classId: string }> }) {
  ...
  return <ClassPeopleContent initialParticipants={preview?.classParticipants[classId]} identityCandidates={preview?.identityCandidates} assignmentPolicy={preview?.participantAssignmentPolicies?.[classId]} />;
}
