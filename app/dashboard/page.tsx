...
export default function DashboardPage() {
  ...
  const handleSingleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isGmail = data.email.endsWith("@gmail.com");
      setSuccessMsg(
        `Berhasil mendaftarkan ${data.email} (${isGmail ? "Silent Whitelist, tanpa email" : "Email undangan terkirim ke alamat non-Gmail"}).`
      );
      setInviteName("");
      setInviteEmail("");
      setInviteWhatsapp("");
  ...
  const handleBulkInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setBulkResult(data);
      setSuccessMsg(`Whitelist massal selesai. ${data.invited.length} berhasil diundang.`);
      setRawEmails("");
      fetchUsersList();
    } catch (err: any) {
        ...
        <div className="space-y-6">
          ...
          {profile?.role === "admin" && adminActiveTab === "admin" && (
            <div className="space-y-6">
              ...
              {/* Nested Sub-Tabs inside Admin View */}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-3xs text-muted-foreground">
                        Kelola data, edit info, suspend, atau hapus massal akun pengguna.
                      </p>
                    </div>
                        ...
                        <tbody className="divide-y divide-border">
                          {usersList.map((user) => (
                            <tr key={user.id} className="hover:bg-secondary/15 transition-colors">
                              ...
                              <td className="py-3 px-3 font-mono text-2xs max-w-[160px] truncate text-muted-foreground">
                                {user.email}
                              </td>
              ...
              {/* TAB 2: INVITE / WHITELIST USER PANELS */}
              {adminSubTab === "invite" && (
                <div className="space-y-6">
...
// Custom simple close icon replacement to avoid missing icon import
